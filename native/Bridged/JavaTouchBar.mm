...
@interface JavaTouchBar() {
    ...
    NSArray<JavaTouchBarItem*> *_javaTouchBarItems;
}
...
-(NSArray<JavaTouchBarItem*>*) getTouchBarItems:(JNIEnv*)env reload:(BOOL)reload {
    @synchronized(_javaTouchBarItems) {
        if(_javaTouchBarItems == nil || [_javaTouchBarItems count] == 0 || reload) {
            _javaTouchBarItems = [[NSMutableArray alloc] init]; // reinitialize
            
            jobject touchBarItems = JNIContext::CallObjectMethod(env, _javaRepr, "getItems", "java/util/List");
            if(touchBarItems == nullptr) {
                return _javaTouchBarItems;
            }
            ...
            jclass groupItemCls = JNIContext::GetOrFindClass(env, "dev/kocken/nativebridge/item/GroupTouchBarItem");
            jclass popoverItemCls = JNIContext::GetOrFindClass(env, "dev/kocken/nativebridge/item/PopoverTouchBarItem");
            ...
            while (env->CallBooleanMethod(touchBarItemIterator, hasNextMid)) {
                ...
                [((NSMutableArray*)_javaTouchBarItems) addObject:item];
            }
        }
