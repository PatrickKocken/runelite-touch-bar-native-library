...
@implementation JavaTouchBarBridge
...
+(void) setTouchBar:(JNIEnv*)env cls:(jclass)cls viewOrWindowPointerValue:(long)viewOrWindowPointerValue touchBar:(jobject)touchBar {
    @try {
        ...
        JavaTouchBarResponder *javaPreviousTouchBarResponder = [windowMapping objectForKey:nsWindow];
        if(javaPreviousTouchBarResponder == nil && touchBar == nullptr) {
            return;
        }
        
        if(touchBar == nullptr) {
            [javaPreviousTouchBarResponder setTouchBar:nil window:nsWindow];
            [windowMapping removeObjectForKey:nsWindow];
        }
        else {
            if(javaPreviousTouchBarResponder != nil) {
                // ensure any old references get destroyed
                [javaPreviousTouchBarResponder setTouchBar:nil window:nsWindow];
                [windowMapping removeObjectForKey:nsWindow];
            }
            
            JavaTouchBarResponder *javaTouchBarResponder = [[JavaTouchBarResponder alloc] init];
            [windowMapping setObject:javaTouchBarResponder forKey:nsWindow];
            
            JavaTouchBar *javaTouchBar = [[JavaTouchBar alloc] init];
            javaTouchBar.javaRepr = touchBar;
            
            [javaTouchBarResponder setTouchBar:javaTouchBar window:nsWindow];
        }
    }
    @catch(NSException* e) {
